...
Rewrites the package name, the CLI entry point, the ``MKT_GOV_`` env-var prefix, and the
...
changed), then run ``make gate``. See docs/ADOPTING.md for the full checklist (rule seed,
IdP wiring, region, eval golden set) that this script does NOT do for you.
"""
...
_OLD_PACKAGE = "marketing_compliance_gate"
_OLD_CLI = "mkt-gov"
_OLD_ENV_PREFIX = "MKT_GOV_"
_OLD_RESOURCE = "marketing-compliance-gate"
_OLD_DIST = "marketing-compliance-gate"
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``MKT_GOV_`` when it prefixes an UPPER token
        # (MKT_GOV_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument("--resource", required=True, help="new resource id stem, e.g. acme-ad-review")
    ...
    print("Then finish the human steps in docs/ADOPTING.md (rule seed, IdP, region, eval set).")
    return 0
