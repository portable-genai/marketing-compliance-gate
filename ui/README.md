...
| `components/ModelPills.tsx`, `lib/answer-provenance.mjs` | The two pills at the top right of every page: the model that answered the last request (the configured `generator_model` from `/healthz`, dimmed, until one has), and `Search` when that answer used an online search tool. One `window.fetch` wrapper reads the service's `X-Answered-By` / `X-Search-Used` headers off responses from `API_BASE`; the service lists both in its CORS `expose_headers`. `tests/answer-provenance.test.mjs` covers the wrapper. |
