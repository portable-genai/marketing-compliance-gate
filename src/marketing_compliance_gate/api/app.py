...
from hex_service_kit.provenance import ANSWERED_BY_HEADER, SEARCH_USED_HEADER
...
app.add_middleware(
    ...
    # The console calls this service directly (cross-origin when standalone), and a browser
    # hides every response header CORS does not list. Without these two the model pills would
    # stay on the configured model forever, with every server-side test green.
    expose_headers=[ANSWERED_BY_HEADER, SEARCH_USED_HEADER],
)
...
# configuration says would. A request that noted nothing (the deterministic rule engine alone,
# or a narration that failed and fell back) sends neither, and the pill keeps showing the
