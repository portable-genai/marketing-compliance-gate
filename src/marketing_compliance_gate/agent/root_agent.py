...
D6 grounds on the per-market rule set served by the ``RuleProviderPort`` (the versioned rule
pack bundled in the package; the shared ``enterprise-knowledge-base`` under ``platform``), not
public-web research, so it carries no ``google_search`` grounding sub-agent. It exposes only the
review (maker) tool; approval is a human checker action and is never an agent skill.
...
def build_root_agent(settings: Settings | None = None) -> LlmAgent:
    ...
    # No temperature: the agent drafts and explains around tools that decide, so it samples at
    # the model's own default rather than at a number chosen here.
