"""ReviewRouterPort: the boundary that routes an escalated compliance review to Hrz7 (rule R8).

Mkt6 **is** the marketing maker-checker gate: every clear or block :class:`Review` sets
``requires_human_review`` and its ``ApprovalRecord`` starts PENDING. Rule R8 says a
...
hand-off, wired ONLY on the review/maker producer path: the agent proposes (maker), a qualified
human disposes (checker), and the console is where that disposition happens. The domain stays pure:
...
class ReviewRouterPort(Protocol):
    def route(self, review: Review, *, maker: str, tenant: str = "") -> None:
        """Route an escalated compliance review to Hrz7 for human review (idempotent is ideal)."""
        ...

    def route_assessment(
        self, assessment: SubstantiationAssessment, *, maker: str, tenant: str = ""
    ) -> None:
        """Route an escalated green-claim substantiation to Hrz7 (rule R8).
