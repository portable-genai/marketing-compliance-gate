...
class LlmRequest:
    ...
    #: ``None`` sends no temperature at all: the call samples at the model's own default, which
    #: is right for narration and the only thing a model that rejects the parameter accepts.
    #: A call whose output is extracted, classified, scored or compared passes ``0.0`` itself.
