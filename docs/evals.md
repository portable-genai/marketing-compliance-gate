...
|---|---|---|---|
| `citation_accuracy` | 1 | a rate; needs 0 positives | Every finding cites a rule in the active rule set (no foreign citations). |
...
| `review_safety` | 1 | a rate; needs 0 positives | Every non-compliant review must require human review (maker-checker). |
| `rule_coverage` | 1 | a rate; needs 0 positives | Fraction of applicable rules that produced a finding. |
| `substantiation_accuracy` | 1 | a rate; needs 0 positives | Fraction of golden green-claim assets whose substantiation verdict matches the independent golden verdict. |
