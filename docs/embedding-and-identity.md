...
| `MKT_GOV_IAP_TENANT_DOMAINS_JSON` | backend | (unset) | JSON object mapping a verified sign-in domain to the tenant the consent store is read and written under. Unset keeps `hd` as the tenant; set and empty refuses. Load-bearing for `POST /v1/review` too: a review reads its audience subject's consent records under this tenant, so an unmapped caller reads none and fails every market consent rule. |
| `MKT_GOV_IAP_MACHINE_TENANTS_JSON` | backend | (unset) | JSON object mapping an exact service-account address to a tenant, for a programmatic caller. Never keyed by domain: every account in a project shares one. Without it a machine caller resolves no tenant, so it reads no consent and its reviews are all non-compliant. |
