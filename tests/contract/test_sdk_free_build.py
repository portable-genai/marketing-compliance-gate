...
bindings the settings file really has (identity, the review router, and the Firestore consent
and evidence stores), ``sys.modules`` is shared with everything else in the session so its
verdict depends on what imported what first, and it says nothing about the ``platform``
family, which reuses several of those same modules, or about the ``vertexai`` root.
