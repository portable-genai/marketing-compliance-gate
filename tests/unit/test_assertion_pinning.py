...
def _adapter_module() -> object:
    ...
    return importlib.import_module("marketing_compliance_gate.adapters.gcp.iap_identity")
...
class TestTheAudienceReadKeepsItsThreeStates:
    ...
    ``MKT_GOV_IAP_AUDIENCE`` has three states: unset, set-and-empty, set-and-valid. The adapter
    ...
    def _adapter(self, monkeypatch: pytest.MonkeyPatch, raw: str | None) -> object:
        monkeypatch.delenv("MKT_GOV_IAP_AUDIENCE", raising=False)
        if raw is not None:
            monkeypatch.setenv("MKT_GOV_IAP_AUDIENCE", raw)
