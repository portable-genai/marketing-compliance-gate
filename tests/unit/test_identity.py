...
def test_default_persona_when_no_header() -> None:
    ...
    assert principal.subject == "demo.reviewer@brand.example"
...
def test_persona_header_is_case_insensitive() -> None:
    # RequestContext lower-cases header-name lookups, and the adapter lower-cases the
    # persona-id value, so a host that sends X-Dev-Persona: Other-Tenant still resolves.
    principal = _adapter().resolve(RequestContext(headers={"x-dev-persona": "Other-Tenant"}))
    assert principal.tenant == "other-brand"


def test_cross_tenant_persona_available() -> None:
    principal = _adapter().resolve(RequestContext(headers={"x-dev-persona": "other-tenant"}))
    assert principal.tenant == "other-brand"
    assert principal.subject == "user@other-tenant.example"
